...
export const BehavioralPerformanceDialog = ({
  ...
  const getSquareColor = (index: number, total: number, isAboveAverage: boolean) => {
    const baseColor = isAboveAverage 
      ? showNonCompliance ? '#FF6B8A' : '#78c38e'
      : showNonCompliance ? '#78c38e' : '#FF6B8A';
          ...
          <div className="space-y-4">
            ...
            {sortedConsultants.slice(0, teamAverageIndex).map((consultant, index) => (
              <div key={consultant.name} className="grid grid-cols-[60px_300px_200px_200px_200px] items-center gap-4 bg-[#1E1E1E] p-4 rounded">
                ...
                <span className="text-right">{consultant.high_need_score}</span>
                <span className="text-right">{consultant.all_score}</span>
                <span className="text-right">{consultant.percentage_high_need}</span>
              </div>
            ))}
            ...
            <div className="grid grid-cols-[60px_300px_200px_200px_200px] items-center gap-4 bg-[#1E1E1E] p-4 rounded">
              ...
              <span className="text-right">{averageResult.high_need_score}</span>
              <span className="text-right">{averageResult.all_score}</span>
